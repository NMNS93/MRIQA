...
Usage: rename.py [-h] [--version] [-c config] -i indir
...
class RenameQA():
    ...
    def __init__(self, indir, outdir, unnmdir, config):
        ...
        self.logger.info('Processing DICOM files in %s', self.indir)
        for dcm_file in self.filepath(self.indir):
            self.file_count += 1
            self.process(dcm_file)
        # Report number files processed
        self.logger.info('Found %d files. Renamed %d/%d DICOM files', self.file_count, self.dcm_rename_count, self.dcm_count)
    ...
    def copy_unnamed(self, infile):
        """Copy invalid dicom files to the 'unnamed' directory (self.unnmdir)
        Args:
            infile - A file which is unable to be renamed by the script
        """
        shutil.copy(infile, self.unnmdir)
        self.logger.warning('Unnamed (file,SeriesDescription): %s', os.path.basename(infile))
...
def main(args):
    ...
    logger.info('BEGIN')
    logger.info('Input directory is %s, Output directory is %s', opts.i, outdir)
