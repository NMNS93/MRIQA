...
Usage: uniformity.py [-h] [--version] [-c config] [-i indir]
...
def cli(args):
    ...
    parser.add_argument('-c', type=str, metavar='config', help='config file containing filename regular expressions',
                        default=pkg_resources.resource_filename(__name__, "data/config.ini"))
...
class UniformityQA():
    ...
    def __init__(self, indir, outdir, config):
        ...
        for UniformityTuple in self.files:
            self.logger.info('Reading file: %s', UniformityTuple.filename)
    ...
    @staticmethod
    def fraction_uniformity(profile, roi_mean):
        """Return the fraction and fraction uniformity of an array of pixel values.
        Args:
            profile: A list of pixel values generated by taking the mean of an ROI profile
            roi_mean: The mean pixel value of the central ROI
        Returns:
            (fraction, uniformity)
        """
        # Calculate the the aboslute difference between each vertical profile and the central ROI mean.
        abs_dif = list(map(lambda x: abs(x - roi_mean), profile))
        # Get the fraction count. This is the number of profiles where abs_diff < (profile_mean *0.1)
        fraction = sum([(diff < (prof*0.1)) for prof, diff in zip(profile, abs_dif)])
        fraction_uniformity = fraction/160
        return fraction, fraction_uniformity

    @staticmethod
    def array_as_string(array):
        """Convert an array to a comma-separated string for writing to output CSV file.
        Args:
            array: A python list containing integers
        """
    ...
    def measure(self, filename, pydicom_image):
        ...
        Returns:
            ...
            Keys: filename, dimensions, midpoint, central_roi_mean, horizontal_roi_profile,
        ...
        # Calculate the ROI profiles
        # Slice the horizontal ROI (160x10) from the image
        horizontal_roi = pydicom_image[midy-5:midy+5, midx-80:midx+80]
        # Slice the vertical ROI (10x160) from the image
        vertical_roi = pydicom_image[midy-80:midy+80, midx-5:midx+5]

        # Loop through the horizontal ROI x-axis, collect y-axis values and calculate the averages
        horizontal_profile_data = ([np.average(horizontal_roi[:,i]) for i in range(horizontal_roi.shape[1])])
        # Loop through the vertical ROI y-axis, collect x-axis values and calculate the averages
        vertical_profile_data = ([np.average(vertical_roi[i,:]) for i in range(vertical_roi.shape[0])])

        # Measure the fraction and uniformity for the two profiles
        horizontal_fraction, horizontal_uniformity = self.fraction_uniformity(horizontal_profile_data, central_roi_mean)
        vertical_fraction, vertical_uniformity = self.fraction_uniformity(vertical_profile_data, central_roi_mean)

        # Calculate horizontal and vertical single line profiles
        horizontal_sl_profile = [pydicom_image[midy,x] for x in range(pydicom_image.shape[1])]
        vertical_sl_profile = [pydicom_image[y, midx] for y in range(pydicom_image.shape[0])]
        ...
        uniformity_stats = {
            ...
            "horizontal_uniformity": horizontal_uniformity,
            "vertical_fraction": vertical_fraction,
            "vertical_uniformity": vertical_uniformity,
            "horizontal_roi_profile": horizontal_profile_data,
            "horizontal_sl_profile": horizontal_sl_profile,
            "vertical_roi_profile": vertical_profile_data,
            "vertical_sl_profile": vertical_sl_profile,
            "image": pydicom_image,
            "contour": contour
            }
    ...
    def write(self, stats):
        ...
        cv2.rectangle(ROI_image, (midx-5,midy-80), (midx+5, midy+80), (255,255,255), 1)
        ...
        # Filter input stats dict to desired fields
        fields = ["filename", "central_roi_mean", "horizontal_fraction", "horizontal_uniformity",
                  "vertical_fraction", "vertical_uniformity", "vertical_roi_profile", "horizontal_roi_profile",
                  "vertical_sl_profile", "horizontal_sl_profile"]
        filtered_stats = {k:v for k, v in stats.items() if k in fields}
        # Set profile lines as strings
        for field, value in filtered_stats.items():
            if re.search(".*profile.*", field):
                filtered_stats[field] = self.array_as_string(value)
        ...
        with open(stats_outfile, 'a') as csvfile:
            writer = csv.DictWriter(csvfile, fieldnames=filtered_stats.keys())
            if not outfile_bool:
                writer.writeheader()
